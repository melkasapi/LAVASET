...
void GBCP(int M, int N, double* Labels, double* Data, int minleaf, int num_labels, double* bcvar, double* bcval){
    ...
    for(j = 0;j<M;j++) {
        cl = Labels[j];
        diff_labels[cl]++; // Changed from "diff_labels[cl-1]++"
    ...
    for(i = 0;i<N;i++){
        ...
        for(mj = 0 ; mj<minleaf-1;mj++){
            cl=sorted_labels[mj];
            diff_labels_l[cl]++;  // Changed from "diff_labels_l[--cl]++"
            diff_labels_r[cl]--;  // Changed from "diff_labels_r[cl]--"
        }
